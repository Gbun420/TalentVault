...
export default function JobseekerProfileForm({
  ...
  return (
    <form className="space-y-6" onSubmit={onSubmit}>
      ...
      <div className="card space-y-4 p-6">
        <h2 className="text-lg font-semibold text-slate-900">Profile file (optional)</h2>
        <input
          name="profile_file"
          type="file"
        ...
        />
        {initialContact?.profile_storage_path ? (
          <p className="text-xs text-slate-600">
            Existing file path: {initialContact.profile_storage_path}
          </p>
        ) : (
          <p className="text-xs text-slate-600">Upload a PDF file (e.g., your resume) to share on unlock.</p>
        )}
      </div>
